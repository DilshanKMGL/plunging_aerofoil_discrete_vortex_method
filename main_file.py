...
# --------------------------------------------------------------------------------
ff.make_file(joukowski_parameter, center, velocity, aoa, time_step, iteration, distance, angle,
             pl_amplitude, pl_frequency)
# --------------------------------------------------------------------------------
fun_1 = ff.plunging(pl_amplitude, pl_frequency, current_time)  # return position and velocity
func = cmath.polar(fun_1[1] + velocity)   # aoa should calculate before any function run
velocity = func[0]
aoa = aoa + func[1]
...
# --------------------------------------------------------------------------------
# write in csv file
ff.write_array(circulation, vortex_strength, vortex_z, vortex_zeta, 1)
# --------------------------------------------------------------------------------
...
velocity_function = [(vortex_function -
                      sp.diff(ff.get_vortex(center, vortex_zeta[index], radius, vortex_strength[index]), zeta)).evalf()
                     for index in range(len(vortex_strength))]
velocity_function = np.multiply(velocity_function, first_derive) - \
                    (np.multiply(vortex_strength, 1j) / sp.pi * second_derive / first_derive)
